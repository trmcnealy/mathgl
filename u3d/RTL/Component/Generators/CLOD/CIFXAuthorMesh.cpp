...
IFXRESULT CIFXAuthorMesh::Copy(IFXREFIID interfaceID, void** ppOutNewAuthorMesh)
{
	...
	{
		U32 uTemp = 0;
		uTemp = pNewAuthorMesh->SetMaxResolution(m_MaxResolution);
	...
	{
		U32 uTemp = 0;
		uTemp = pNewAuthorMesh->SetMinResolution(m_MinResolution);
...
U32 CIFXAuthorMesh::SetResolution(U32 r)
{
	...
	while (r > m_CurMeshDesc.NumPositions)
	{
		...
		while(nfu--)
		{
			...
			if ((pfu->Attribute >= IFXAuthorFaceUpdate::Tex0) && 
				(pfu->Attribute < (IFXAuthorFaceUpdate::Tex0+IFX_MAX_TEXUNITS)))
			{
