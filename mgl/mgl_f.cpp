...
void mgl_set_bar_width_(uintptr_t *gr, float *width)
{	_GR_->BarWidth = *width;	}
...
void mgl_set_mark_size_(uintptr_t *gr, float *size)
{	_GR_->MarkSize = *size;	}
/// Set size of arrows mglGraph::ArrowSize.
void mgl_set_arrow_size_(uintptr_t *gr, float *size)
{	_GR_->ArrowSize = *size;	}
/// Set default font size mglGraph::FontSize.
void mgl_set_font_size_(uintptr_t *gr, float *size)
{	_GR_->FontSize = *size;	}
/// Set default alpha value mglGraph::AlphaDef.
void mgl_set_alpha_default_(uintptr_t *gr, float *alpha)
{	_GR_->AlphaDef = *alpha;	}
...
{	_GR_->Alpha(*enable);	}
/// Set the fog on/off.
void mgl_set_fog_(uintptr_t *gr, float *d, float *dz)
{	_GR_->Fog(*d, *dz);	}
...
void mgl_add_light_rgb_(uintptr_t *gr, int *n, float *x, float *y, float *z, int *infty,
...
void mgl_identity_(uintptr_t *gr)
{	_GR_->Identity();	}
/// Clear up the frame
...
void mgl_set_ticks_dir_(uintptr_t *gr, char *dir, float *d, int *ns, float *org)
{	_GR_->SetTicks(*dir, *d, *ns, *org);	}
/// Set ticks interval mglGraph::dx, mglGraph::dy, mglGraph::dz.
void mgl_set_ticks_(uintptr_t *gr, float *DX, float *DY, float *DZ)
...
void mgl_set_caxis_(uintptr_t *gr, float *C1,float *C2)
{	_GR_->CAxis(*C1,*C2);	}
...
void mgl_set_axis_(uintptr_t *gr, float *x1, float *y1, float *z1, float *x2, float *y2, float *z2, float *x0, float *y0, float *z0)
...
void mgl_box_rgb_(uintptr_t *gr, float *r, float *g, float *b, int *ticks)
...
/// Print the label \a text for axis \a dir.
void mgl_label_ext_(uintptr_t *gr, const char *dir, const char *text, int *pos, float *size, float *shift,int,int l)
{
	char *s=new char[l+1];	memcpy(s,text,l);	s[l]=0;
...
void mgl_ball_str_(uintptr_t *gr, float *x, float *y, float *z, const char *col,int)
{	_GR_->Ball(mglPoint(*x,*y,*z),*col);	}
/// Draws the line between points with style \a sch.
void mgl_line_(uintptr_t *gr, float *x1, float *y1, float *z1, float *x2, float *y2, float *z2,
...
void mgl_puts_(uintptr_t *gr, float *x, float *y, float *z,const char *text,int l)
{
	char *s=new char[l+1];	memcpy(s,text,l);	s[l]=0;
	_GR_->Puts(mglPoint(*x,*y,*z),s);	delete []s;
}
/// Print string \a str in position \a p auintptr_t direction \a d with font size \a size.
void mgl_puts_dir_(uintptr_t *gr, float *x, float *y, float *z, float *dx, float *dy, float *dz, const char *text, float *size, int l)
{
	char *s=new char[l+1];	memcpy(s,text,l);	s[l]=0;
...
void mgl_text_(uintptr_t *gr, float *x, float *y, float *z,const char *text,int l)
{
	char *s=new char[l+1];	memcpy(s,text,l);	s[l]=0;
	_GR_->Text(mglPoint(*x,*y,*z),s);	delete []s;
}
/// Print string \a str in position \a p with font size \a size.
void mgl_puts_ext_(uintptr_t *gr, float *x, float *y, float *z,const char *text,
...
/// Print unrotated string \a str in position \a p with font size \a size.
void mgl_text_ext_(uintptr_t *gr, float *x, float *y, float *z,const char *text,
					const char *font, float *size,const char *dir,int l,int n,int)
{
	char *s=new char[l+1];	memcpy(s,text,l);	s[l]=0;
	char *f=new char[n+1];	memcpy(f,font,n);	f[n]=0;
...
void mgl_set_axis_3d_(uintptr_t *gr, float *x1, float *y1, float *z1, float *x2, float *y2, float *z2)
{	_GR_->Axis(mglPoint(*x1,*y1,*z1),mglPoint(*x2,*y2,*z2));	}
void mgl_set_axis_2d_(uintptr_t *gr, float *x1, float *y1, float *x2, float *y2)
...
void mgl_cone_(uintptr_t* gr, float *x1, float *y1, float *z1, float *x2, float *y2, float *z2, float *r1, float *r2, const char *stl, int *edge, int l)
{
	char *s=new char[l+1];	memcpy(s,stl,l);	s[l]=0;
